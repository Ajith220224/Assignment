  ...
  return (
    <header className="sticky top-0 z-40 bg-paper/95 backdrop-blur border-b border-ink/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4">
        <Link
          to="/"
          className="font-display text-2xl font-semibold text-ink tracking-tight shrink-0"
        >
          FlipKart
        </Link>
        ...
        <form
          ...
          className="hidden md:flex flex-1 max-w-md items-center border border-ink/15 rounded-full px-4 py-2 bg-white"
        >
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-ink/80">
          ...
          {user ? (
            ...
            <Link
              to="/login"
              className="bg-ink text-paper px-4 py-2 rounded-full hover:bg-plum transition-colors"
            >
              Login
            </Link>
          )}
      ...
      {open && (
        <div className="md:hidden px-4 pb-4 flex flex-col gap-3 text-sm font-medium">
          <form
            onSubmit={handleSearch}
            className="flex items-center border border-ink/15 rounded-full px-4 py-2 bg-white"
          >
            <FiSearch className="text-ink/40 mr-2" />
            <input
              value={search}
          ...
          {user ? (
            <button onClick={handleLogout} className="text-left text-clay">
              Logout
            </button>
          ) : (
            <Link
